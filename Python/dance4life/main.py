...
from sensor.weather import get_weather
from sensor.air_quality import get_air_quality
from sensor.traffic import get_traffic
from config.config import COLLECTORS_INTERVAL, COLLECTORS_WEATHER_COLLECTION_FIREBASE, COLLECTORS_AIR_QUALITY_COLLECTION_FIREBASE, COLLECTORS_TRAFFIC_COLLECTION_FIREBASE, DANCE4LIFE_ACTIVITY_COLLECTION_FIREBASE
...
def run_collectors():
    while True:
        try:
            ...
            save_data_from_collectors(COLLECTORS_WEATHER_COLLECTION_FIREBASE, weather)
            save_data_from_collectors(COLLECTORS_AIR_QUALITY_COLLECTION_FIREBASE, air)
            save_data_from_collectors(COLLECTORS_TRAFFIC_COLLECTION_FIREBASE, traffic)
        ...
        time.sleep(COLLECTORS_INTERVAL)

if __name__ == "__main__":
    try:
        # Correr collectors numa thread separada (é função síncrona)
        collectors_thread = threading.Thread(target=run_collectors, daemon=True)
        collectors_thread.start()

        # Correr o loop principal assíncrono
        asyncio.run(run())
